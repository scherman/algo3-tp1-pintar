...
int main() {
    int n = 200;
//    int arr[n] = {10, 3, 10, 11, 10, 4, 11, 10};
...
//    int arr[n] = {45,39,34,24,99,91,85,98,93,18,6,5,9,90,72,72,58,82,15,5,95,38,60,50,94,11,84,27,60,45,57,67,23,32,44,82,85,25,5,22,74,80,4,40,100,25,79,64,71,53,80,10,97,27,77,22,12,26,50,78,98,75,25,72,31,3,69,69,67,20,71,21,34,47,37,47,96,48,100,95,99,60,96,98,48,55,96,71,99,89,44,87,29,78,60,35,33,63,2,69,65,3,10,76,65,5,67,51,26,94,94,84,88,9,51,48,81,41,49,43,69,8,13,18,57,65,93,53,71,31,65,23,56,65,60,89,63,87,9,4,22,1,7,51,60,61,97,76,38,93,51,8,46,57,49,7,52,22,43,7,89,32,71,86,73,50,50,32,58,96,9,35,6,5,60,19,36,71,10,89,54,96,57,66,56,83,57,60,1,35,23,55,26,66,89,39,95,82,25,4}; // t20 ok
    ...
    int res2 = pintarDinamicoTD2(arr, n, n, 0, 0, m);
    int res3 = pintarDinamicoTD(arr, n);
    std::cout << "Rta: " << res << ", " << res2 << ", " << res3 << std::endl;
//    std::list<int> rojas1;
...
////
//
//    int res3 = pintarDinamico(arr, 0, n, rojas3, azules3, sinPintar3, minSinPintar3, m);
...
/*
 * f(n, r, a): ""
 * f(i, r, a): "cantidad de elementos que ya analicé, "
 * A: problema original
...
int pintarDinamicoTD2(int A[], int n, int i, int r, int a, int ***m) {
    ...
    if (i == 0) {
        ...
        int ultimoSubproblema = A[i - 1];

        // Llamada al hijo izquierdo (si es válido): pinto 'ultimoSubproblema' de rojo
        int minRamaR = n;
        if (r == 0 || A[r-1] < ultimoSubproblema) {
            minRamaR = pintarDinamicoTD2(A, n, i - 1, i, a, m);
        }

        // Llamada al hijo medio (si es válido): pinto 'ultimoSubproblema' de azul
        int minRamaA = n;
        if (a == 0 || A[a-1] > ultimoSubproblema) {
            minRamaA = pintarDinamicoTD2(A, n, i - 1, r, i, m);
        }

        // Llamada al hijo derecho: dejo 'ultimoSubproblema' sin pintar
        int minRamaSP = pintarDinamicoTD2(A, n, i - 1, r, a, m) + 1;

        min = min3(minRamaR, minRamaA, minRamaSP); // Guardo lo calculado
    }
    m[i][r][a] = min;
    return min;
}
