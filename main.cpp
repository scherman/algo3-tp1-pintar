...
//    int res = pintarDinamicoBU2(arr, n);
//    int ***m = new int**[n+1];
//    for(int k = 0; k < n+1; ++k) {
//        m[k] = new int*[n+1];
//        for(int r = 0; r < n+1; ++r) {
...
//        }
//    }
    int res2 = pintarDinamicoTDEnPosicion2(arr, n);
    int res3 = pintarDinamicoBU2(arr, n);
    std::cout << "Rta: " << res2 << ", " << res3 << std::endl;
...
int pintarDinamicoTD2(int A[], int n, int i, int r, int a, int ***m) {
    ...
    if (i == 0) {
        ...
        int primerElem = A[n - i];

        // Llamada al hijo izquierdo (si es válido): pinto 'primerElem' de rojo
        int minRamaR = n;
        if (r == 0 || A[r-1] < primerElem) {
            minRamaR = pintarDinamicoTD2(A, n, i - 1, n - i + 1, a, m);
        }

        // Llamada al hijo medio (si es válido): pinto 'primerElem' de azul
        int minRamaA = n;
        if (a == 0 || A[a-1] > primerElem) {
            minRamaA = pintarDinamicoTD2(A, n, i - 1, r, n - i + 1, m);
        }
...
//                    m[i][r][a] = n;
//                } else {
                    int minRamaR = r == 0 || A[r-1] < primerElem ? m[i-1][n-i+1][a] : n;
                    int minRamaA = a == 0 || A[a-1] > primerElem ? m[i-1][r][n-i+1] : n;
